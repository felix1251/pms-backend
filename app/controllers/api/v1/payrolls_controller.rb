class Api::V1::PayrollsController < PmsDesktopController
  ...
  def payroll_data
    ...
    sql_time_keeping_hours_sum += " WHERE employee_id = emp.id AND date(start_time) = only_date AND duty_type = 'X' LIMIT 1), 1))"
    ...
    sql_time_undertime_sum += " FROM employee_schedules WHERE employee_id = emp.id AND DATE(start_time) = only_date AND duty_type = 'X' LIMIT 1), 0)"
    ...
    sql_time_undertime_sum += " IF(date > (SELECT start_time FROM employee_schedules WHERE DATE(start_time) = only_date AND duty_type = 'X' LIMIT 1), DATE_FORMAT(date, '%Y-%m-%d %H:%i'), (SELECT start_time FROM employee_schedules WHERE DATE(start_time) = only_date AND duty_type = 'X' LIMIT 1)),"
    sql_time_undertime_sum += " IF(next_date < (SELECT end_time FROM employee_schedules WHERE DATE(start_time) = only_date AND duty_type = 'X' LIMIT 1), DATE_FORMAT(next_date, '%Y-%m-%d %H:%i'), (SELECT end_time FROM employee_schedules WHERE DATE(start_time) = only_date AND duty_type = 'X' LIMIT 1))"
    ...
    sql_payed_overtime_hours_sum += " ELSE DATE_FORMAT(ov.start_date, '%Y-%m-%d %H:%i') END,"
		...
		sql_payed_overtime_hours_sum += " WHERE ov.status = 'A' AND ov.employee_id = emp.id AND ov.billable = 1"
    ...
    sql_payed_offset_hours_sum = " COALESCE((SELECT" 
		sql_payed_offset_hours_sum += " SUM(8)"
    ...
    sql_payed_spec_hol_hours_sum = " (SELECT IFNULL(SUM((SELECT"
		...
		sql_payed_spec_hol_hours_sum += " LEFT JOIN employee_schedules as emp_sch ON emp_sch.employee_id = emp.id AND DATE(emp_sch.start_time) = spec_hol.date AND emp_sch.duty_type = 'X'" 
